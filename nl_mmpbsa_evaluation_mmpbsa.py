...
def tleap(ff, pdbid, output_dir):
    ...
    return (receptor_prmtop, receptor_inpcrd, receptor_pdb, ligand_prmtop, ligand_inpcrd, ligand_pdb, complex_prmtop,
...
'''
        ...
        if solvation_models in ['no', 'implicit1', 'implicit2', 'implicit5', 'implicit7', 'implicit8']:
            command = f'timeout 120m mpirun -np 48 MMPBSA.py -O -i {mmgbsain_file} -cp {complex_prmtop} -rp {receptor_prmtop} -lp {ligand_prmtop} -y {inpcrd} -o {os.path.join(output_dir, "FINAL_RESULTS_MMPBSA_100ns.dat")} -prefix {os.path.join(output_dir, "_MMPBSA_")}  > {mmgbsalog_file} && '
            command += f'cd %s && MMPBSA.py --clean > /dev/null' % output_dir  ## 清除临时文件

        elif solvation_models == 'explicit':
            command = f'timeout 120m mpirun -np 48 MMPBSA.py -O -i {mmgbsain_file} -sp {complex_solv_prmtop} -cp {complex_prmtop} -rp {receptor_prmtop} -lp {ligand_prmtop} -y {inpcrd} -o {os.path.join(output_dir, "FINAL_RESULTS_MMPBSA_100ns.dat")} -prefix {os.path.join(output_dir, "_MMPBSA_")} > {mmgbsalog_file} && '
            command += f'cd %s && MMPBSA.py --clean > /dev/null' % output_dir  ## 清除临时文件

        if not os.path.exists(f"{os.path.join(output_dir, 'FINAL_RESULTS_MMPBSA_100ns.dat')}"):
        ...
        if os.path.exists(f"{os.path.join(output_dir, 'FINAL_RESULTS_MMPBSA_100ns.dat')}"):
...
'''
        ...
        if solvation_models in ['no', 'implicit1', 'implicit2', 'implicit5', 'implicit7', 'implicit8']:
            command = f'timeout 120m mpirun -np 48 MMPBSA.py -O -i {mmgbsain_file} -cp {complex_prmtop} -rp {receptor_prmtop} -lp {ligand_prmtop} -y {inpcrd} -o {os.path.join(output_dir, "FINAL_RESULTS_MMPBSA_100ns.dat")} -prefix {os.path.join(output_dir, "_MMPBSA_")} -do {os.path.join(output_dir, "FINAL_DECOMP_MMPBSA.dat")} -use-mdins > {mmgbsalog_file} && '
            command += f'cd %s && MMPBSA.py --clean > /dev/null' % output_dir  ## 清除临时文件
        elif solvation_models == 'explicit':
            command = f'timeout 120m mpirun -np 48 MMPBSA.py -O -i {mmgbsain_file} -sp {complex_solv_prmtop} -cp {complex_prmtop} -rp {receptor_prmtop} -lp {ligand_prmtop} -y {inpcrd} -o {os.path.join(output_dir, "FINAL_RESULTS_MMPBSA_100ns.dat")} -prefix {os.path.join(output_dir, "_MMPBSA_")} -do {os.path.join(output_dir, "FINAL_DECOMP_MMPBSA.dat")} -use-mdins > {mmgbsalog_file} && '
            command += f'cd %s && MMPBSA.py --clean > /dev/null' % output_dir  ## 清除临时文件
        if not os.path.exists(f"{os.path.join(output_dir, 'FINAL_RESULTS_MMPBSA_100ns.dat')}"):
            proc = subprocess.Popen(
                command,
...
def main(num_parts, part_id):
    if os.path.exists('/home/dejun/workspace/NLmmgbsa/scirpts/remained_mmgbsa_task.npy'):
        remained_dict = np.load('/home/dejun/workspace/NLmmgbsa/scirpts/remained_mmgbsa_task.npy',
                                allow_pickle=True).item()
    else:
        p1s, p2s, p3s, p4s, p5s, p6s, p7s, p8s, p9s, p10s, p11s, p12s = [], [], [], [], [], [], [], [], [], [], [], []

        pdbids = ['1ARJ', '1FYP', '1Q8N', '1UTS', '1UUD', '2AU4', '2F4S', '2F4T', '2F4U', '2KGP', '2KTZ', '2KU0',
                  '2KX8', '2L94', '2LWK', '2MXS', '2N0J', '4LVZ', '4LW0', '6HAG', '1QD3', '2O3W', '2O3X', '3NPN', '1BYJ']
        types = ['rna' for _ in pdbids]
        dl_pdbids = ['3S4P', '4LVW', '4LVY', '2BE0']

        for pdbid, type in zip(pdbids[:], types[:]):
            if pdbid not in dl_pdbids:
                if type == 'rna':
                    ...
                    force_fields = {'bsc1': 'leaprc.DNA.bsc1',
                                    'OL15': 'leaprc.DNA.OL15'}
                ...
                if os.path.exists('/home/dejun/workspace/NLmmgbsa/results/%s/%s' % (type, pdbid)):
                    for force_field in ff_keys:
                        ...
                        for solvation_models in solvation_models_ls:
                            ...
                            receptor_prmtop, receptor_inpcrd, receptor_pdb, ligand_prmtop, ligand_inpcrd, ligand_pdb, complex_prmtop, complex_inpcrd, complex_pdb, complex_solv_prmtop, complex_solv_inpcrd, complex_solv_pdb = tleap(
                                ff_dir, pdbid, force_fields[force_field])
                            for md_or_mini in ['md', 'no']:
                                if md_or_mini == 'md':
                                    final_inpcrd = '/home/dejun/workspace/NLmmgbsa/results/%s/%s/%s/md/%s/equil2_100ns_last100ns.mdcrd' % (
                                ...
                                for free_energy_calculation in ['gbsa1', 'gbsa2', 'gbsa5', 'gbsa7', 'gbsa8', 'pbsa']:
                                    for interior_dielectric_constants in [1, 2, 4, 8, 12, 16, 20]:
                                        ...
                                        mmgbsa_working_dir = os.path.join(solvation_dir,
                                                                          free_energy_calculation + '_' + str(
                                                                              interior_dielectric_constants))
                                        if not os.path.exists(
                                                f"{os.path.join(mmgbsa_working_dir, 'FINAL_RESULTS_MMPBSA_100ns.dat')}"):
                                            p1, p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12 = free_energy_calculation, interior_dielectric_constants, mmgbsa_working_dir, complex_solv_prmtop, complex_prmtop, receptor_prmtop, ligand_prmtop, final_inpcrd, complex_pdb, receptor_pdb, ligand_pdb, solvation_models
                                            p1s.append(p1)
                                            p2s.append(p2)
                                            p3s.append(p3)
                                            p4s.append(p4)
        ...
        p1s, p2s, p3s, p4s, p5s, p6s, p7s, p8s, p9s, p10s, p11s, p12s = np.array(p1s), np.array(p2s), np.array(
            p3s), np.array(p4s), np.array(p5s), np.array(p6s), np.array(p7s), np.array(p8s), np.array(p9s), np.array(
            p10s), np.array(p11s), np.array(p12s)
        ls = list(range(len(p1s)))
        random.shuffle(ls)
        p1s, p2s, p3s, p4s, p5s, p6s, p7s, p8s, p9s, p10s, p11s, p12s = list(p1s[ls]), list(p2s[ls]), list(
            p3s[ls]), list(p4s[ls]), list(p5s[ls]), list(p6s[ls]), list(p7s[ls]), list(p8s[ls]), list(p9s[ls]), list(
            p10s[ls]), list(p11s[ls]), list(p12s[ls])
    ...
    p1s, p2s, p3s, p4s, p5s, p6s, p7s, p8s, p9s, p10s, p11s, p12s = remained_dict['p1'], remained_dict['p2'], \
                                                                    remained_dict['p3'], remained_dict['p4'], \
                                                                    remained_dict['p5'], remained_dict['p6'], \
                                                                    remained_dict['p7'], remained_dict['p8'], \
                                                                    remained_dict['p9'], remained_dict['p10'], \
                                                                    remained_dict['p11'], remained_dict['p12']
    print('the total number of remained mmgbsa  claculation task is: %s' % len(p1s))
    num_per_part = len(p1s) // num_parts
    if part_id != num_parts:
        p1s_sel, p2s_sel, p3s_sel, p4s_sel, p5s_sel, p6s_sel, p7s_sel, p8s_sel, p9s_sel, p10s_sel, p11s_sel, p12s_sel = p1s[
                                                                                                                        num_per_part * (
                                                                                                                                    part_id - 1):num_per_part * part_id], p2s[
                                                                                                                                                                          num_per_part * (
                                                                                                                                                                                      part_id - 1):num_per_part * part_id], p3s[
                                                                                                                                                                                                                            num_per_part * (
                                                                                                                                                                                                                                        part_id - 1):num_per_part * part_id], \
                                                                                                                        p4s[
                                                                                                                        num_per_part * (
                                                                                                                                    part_id - 1):num_per_part * part_id], p5s[
                                                                                                                                                                          num_per_part * (
                                                                                                                                                                                      part_id - 1):num_per_part * part_id], p6s[
                                                                                                                                                                                                                            num_per_part * (
                                                                                                                                                                                                                                        part_id - 1):num_per_part * part_id], \
                                                                                                                        p7s[
                                                                                                                        num_per_part * (
                                                                                                                                    part_id - 1):num_per_part * part_id], p8s[
                                                                                                                                                                          num_per_part * (
                                                                                                                                                                                      part_id - 1):num_per_part * part_id], p9s[
                                                                                                                                                                                                                            num_per_part * (
                                                                                                                                                                                                                                        part_id - 1):num_per_part * part_id], \
                                                                                                                        p10s[
                                                                                                                        num_per_part * (
                                                                                                                                    part_id - 1):num_per_part * part_id], p11s[
                                                                                                                                                                          num_per_part * (
                                                                                                                                                                                      part_id - 1):num_per_part * part_id], p12s[
                                                                                                                                                                                                                            num_per_part * (
                                                                                                                                                                                                                                        part_id - 1):num_per_part * part_id]
    ...
    # pool.starmap_async(mmgbsa, zip(p1s_sel, p2s_sel, p3s_sel, p4s_sel, p5s_sel, p6s_sel, p7s_sel, p8s_sel, p9s_sel, p10s_sel, p11s_sel, p12s_sel))
    # pool.close()
    # pool.join()

    for p1_sel, p2_sel, p3_sel, p4_sel, p5_sel, p6_sel, p7_sel, p8_sel, p9_sel, p10_sel, p11_sel, p12_sel in zip(
            p1s_sel, p2s_sel, p3s_sel, p4s_sel, p5s_sel, p6s_sel, p7s_sel, p8s_sel, p9s_sel, p10s_sel, p11s_sel,
            p12s_sel):
