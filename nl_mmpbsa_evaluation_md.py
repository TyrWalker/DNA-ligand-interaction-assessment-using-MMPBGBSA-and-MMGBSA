...
def minimize_or_md(flag, solvation_models, output_dir, complex_prmtop, complex_incrd, complex_pdb):
 ...
 &cntrl
    ...
    ntr=1,
    restraint_wt=1000,
    restraintmask=':1-{count_residue(complex_pdb)-1}',
/
'''
    ...
    for pdbid, type in zip(pdbids[:], types[:]):
        ...
        for force_field in ff_keys:
            ...
            for solvation_models in solvation_models_ls:
                ...
                receptor_prmtop, receptor_inpcrd, receptor_pdb, ligand_prmtop, ligand_inpcrd, ligand_pdb, complex_prmtop, complex_inpcrd, complex_pdb, complex_solv_prmtop, complex_solv_inpcrd, complex_solv_pdb = tleap(ff_dir, pdbid, force_fields[force_field])
                # mini/md/no
                if solvation_models in ['no', 'implicit1', 'implicit2', 'implicit5', 'implicit7', 'implicit8']:
                    final_inpcrd = minimize_or_md(md_or_mini, solvation_models, ff_dir, complex_prmtop, complex_inpcrd, complex_pdb)
                else:
                    final_inpcrd = minimize_or_md(md_or_mini, solvation_models, ff_dir, complex_solv_prmtop, complex_solv_inpcrd, complex_pdb)
